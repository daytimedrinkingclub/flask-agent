...
class ToolsHandler:
    ...
    def process_tool_use(tool_name, tool_input, tool_use_id, chat_id):
        ...
        if tool_name == "consult_subhash":
            ...
            MessageService.save_message(chat_id, "user", content=result, tool_use_id=tool_use_id, tool_result=result)
            return result
            ...
            MessageService.save_message(chat_id, "user", content=action_status, tool_use_id=tool_use_id, tool_result=action_status)
            ...
            MessageService.save_message(chat_id, "user", content=instruction_status, tool_use_id=tool_use_id, tool_result=instruction_status)
            return instruction_status
        elif tool_name == "write_curl_to_database":
            result = ActionService.write_curl_to_database(chat_id, tool_input["curl_as_json"], tool_input["action_name"])
            MessageService.save_message(chat_id, "user", content=result, tool_use_id=tool_use_id, tool_result=result)
            return result
        else:
